    ...
    <div className="flex flex-col relative">
      ...
      >
        <div className="flex justify-between items-center w-full px-2 gap-2 text-sm  border-transparent focus:border-primary">
          ...
          <ChevronDown
            size="20"
            className={`transform ${isSelectOpened ? 'rotate-180' : 'rotate-0'}`}
          />
        </div>
      </div>
      <ul
        className={`absolute top-full mt-1 ${width} bg-muted-foreground overflow-y-auto z-50 border border-dark/20 rounded-md
