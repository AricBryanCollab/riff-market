...
Start Slice `10` Naming and Polish from `ddd/migration-plan.md`.

Recommended first step:

1. Inventory remaining product/listing compatibility names in `src/types`, `src/server`, `src/hooks`, `src/components`, and Prisma-facing adapters.
2. Decide the next low-risk naming slice, keeping persistence rename separate unless the schema work is intentionally in scope.
